...
import { Child, ParentProfile } from "@/utils/types";
...
const ParentHome = () => {
  ...
  } = useQuery({ queryKey: ["children"], queryFn: fetchChildren });
  const {
    data: parentProfile,
    isLoading: loadingProfile,
    refetch: refetchProfile,
    isRefetching: isRefetchingProfile,
  } = useQuery({ queryKey: ["parentProfile"], queryFn: fetchParentProfile });
  ...
  const handleTrackChild = useCallback((child: Child) => {
    router.push(`/parent/track?childId=${child.id}`);
  }, [router]);
